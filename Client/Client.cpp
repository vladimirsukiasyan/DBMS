...
void Client::printCommands() {
    ...
    cout << '|' << " Working with DB: " << setfill(' ') << setw(4) << '|' << " Work inside DB: " << setw(11) << '|' << " Work inside the table: " << setw(21) << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " Create DB - 1 " << setfill(' ') << setw(7) << '|' << " List of tables - 9 " << setfill(' ') << setw(8) << '|' << " The output table - 13 " << setfill(' ') << setw(22) << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " Delete DB - 2 " << setfill(' ') << setw(7) << '|' << " Create the table - 10 " << setfill(' ') << setw(5) << '|' << setfill(' ') << setw(7) << " Insert a record - 14 " << setfill(' ') << setw(23) << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " DB list - 3 " << setfill(' ') << setw(9) << '|' << " Delete the table - 11 " << setfill(' ') << setw(5) << '|' << " Clear the table - 15 " << setfill(' ') << setw(23) << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " Use the DB - 4 " << setfill(' ') << setw(6) << '|' << " Use the table - 12 " << setfill(' ') << setw(8) << '|' << " Column output - 16 " << setfill(' ') << setw(25) << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " Open logs - 5 " << setfill(' ') << setw(7) << '|' << ' ' << setfill(' ') << setw(26) << ' ' << '|' << " Delete a record - 17 " << setfill(' ') << setw(23) << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " Print commands - 6 " << setfill(' ') << setw(2) << '|' << ' ' << setfill(' ') << setw(26) << ' ' << '|' << " Find records(conditions) - 18 " << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " Save DB - 7 " << setfill(' ') << setw(8) << ' ' << '|' << ' ' << setfill(' ') << setw(26) << ' ' << '|' << " Clear column data from all records - 19 "  << "|" << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
    cout << '|' << " Exit - 8" << setfill(' ') << setw(12) << ' ' << '|' << ' ' << setfill(' ') << setw(26) << ' ' << '|' << ' '  << setfill(' ') << setw(44) << '|' << endl;
    cout << '+' << setfill('-') << setw(95) << '+' << endl;
}
