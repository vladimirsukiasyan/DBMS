...
//	day = atoi(strtok_s(token, delims, &token));
//	month = atoi(strtok_s(token, delims, &token));
//	year = atoi(strtok_s(token, delims, &token));
//}
//
...
//	return (1 - (year % 4 + 2) / (year % 4 + 1))*((year % 100 + 2)*(year % 100 + 1)) + (1 - ((year % 400 + 2) / (year % 400 + 1))) ? true : false;
//}
...
//	int sum2 = date.year*(IsLeapYear(date.year) ? 366 : 365) + date.month*GetDaysInMonth(date.month, date.year) + date.day;
...
//	int sum1 = year*(IsLeapYear(year) ? 366 : 365) + month*GetDaysInMonth(month, year) + day;
//	int sum2 = date1.year*(IsLeapYear(date1.year) ? 366 : 365) + date1.month*GetDaysInMonth(date1.month, date1.year) + date1.day;
//	return sum1 - sum2;
//}